...
function pageRender(data) {
    ...
    let newsFill = 0;
    if (news.length < 8) {
        newsFill = 8 - news.length;
    }
    _.forEach(news, (newsItem) => {
        let date = new moment(newsItem.updateTime).format('YYYY-MM-DD');
        let news_item = $(`<div class="news_item"><div class="news_img"><img src="${newsItem.picUrl}" alt=""></div><div class="news_info"><div class="news_item_title"> ${newsItem.name}</div><div class="news_date">${date}</div><div class="news_item_text"> ${newsItem.extensionProfile}</div><div class="news_item_footer"><a href="newsDetail.html?id=${newsItem.id}" class="know_more">查看详情</a></div></div>`);
        news_content.append(news_item)
    })
    for (let i = 0; i < newsFill; i++) {
        news_content.append($(`<div class="fill_item"></div>`))
    }
}